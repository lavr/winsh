...
// TestCommitPreservesConcurrentDestination is the canonical race test:
// the destination already exists, no force, and a later writer is not
// present. commitStage must refuse and the existing destination must
...
func TestCommitRefusesOnExistingNoForce(t *testing.T) {
	...
	// Stage should still exist after a refused commit; cleanup is the
	// caller's job, not commitStage's.
...
func TestCommitMissingStage(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "no-such-stage") && !errors.Is(err, os.ErrNotExist) {
